...
function renderAiRoster(){
 ...
 AI_PROFILES.forEach(p=>{const unlocked=isUnlocked(p);const b=document.createElement('button');b.className=`ai-choice ${selectedAi===p.id?'active':''} ${unlocked?'':'locked'}`;b.dataset.ai=p.id;if(!unlocked)b.dataset.lock=`SOLO ${p.unlock}勝で解放`;b.innerHTML=`<img src="${p.img}" alt="${p.name}"><span class="ai-info"><b>${p.name}</b><small>${p.style}</small><p>${p.blurb}</p></span>`;b.onclick=()=>{if(!unlocked){showToast(`${p.name}はSOLO ${p.unlock}勝で解放`);return}selectedAi=p.id;$$('.ai-choice').forEach(x=>x.classList.remove('active'));b.classList.add('active');playCue('select')};E.aiRoster.appendChild(b)});
...
function renderArena(){
 ...
 for(let n=1;n<=12;n++){const b=document.createElement('button');const remaining=state.remainingSeats.includes(n);b.className=`chair-slot ${remaining?'':'used'} ${selectedSeat===n?'selected':''} ${revealTrapSeat===n?'main-trap-hot':''} ${revealSafeSeat===n?'main-seat-safe':''}`;b.style.setProperty('--i',n-1);b.disabled=!remaining||!(state.canSetTrap||state.canChooseSeat);b.setAttribute('aria-label',`${n}番のイス`);b.innerHTML=`<span class="chair"><i class="chair-back"></i><i class="chair-seat"></i><b class="num">${n}</b></span>`;b.onclick=()=>selectSeat(n);E.chairGrid.appendChild(b)}
}
