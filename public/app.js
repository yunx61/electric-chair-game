...
let ws,state=null,selectedSeat=null,reconnectTimer=null,heartbeatTimer=null,leaveFallbackTimer=null,lastResultKey=null,lastGameOverKey=null,audioCtx=null,ambientNodes=null,resultTimers=[],startTimer=null,sessionReady=false,reconnectAttempts=0,manualSocketClose=false,activeDialog=null,dialogTrigger=null;
...
function scheduleReconnect(){clearTimeout(reconnectTimer);if(manualSocketClose||!state||state.phase==='game_over')return;reconnectAttempts+=1;const delay=Math.min(10000,800*(2**Math.min(reconnectAttempts-1,4)))+Math.floor(Math.random()*300);setConnection('reconnecting');reconnectTimer=setTimeout(resumeSession,delay)}
function connect(onOpen){
 ...
 manualSocketClose=false;setConnection(state?'reconnecting':'connecting');sessionReady=false;const socket=new WebSocket(wsUrl());ws=socket;
 socket.addEventListener('open',()=>{if(ws!==socket)return;clearTimeout(reconnectTimer);reconnectAttempts=0;setConnection('connected');startHeartbeat();onOpen?.()});
 socket.addEventListener('message',onMessage);socket.addEventListener('close',()=>{if(ws!==socket)return;sessionReady=false;clearInterval(heartbeatTimer);ws=null;setConnection('offline');if(state&&state.phase!=='game_over'&&!manualSocketClose){showToast('通信が切れました。再接続中…');scheduleReconnect()}});socket.addEventListener('error',()=>{if(ws===socket)setConnection('offline')});
}
function closeSocket(){manualSocketClose=true;clearTimeout(reconnectTimer);clearInterval(heartbeatTimer);const socket=ws;ws=null;sessionReady=false;if(socket&&[WebSocket.OPEN,WebSocket.CONNECTING].includes(socket.readyState))socket.close(1000,'Client leaving')}
function send(payload){if(ws?.readyState!==WebSocket.OPEN)return false;try{ws.send(JSON.stringify(payload));return true}catch{return false}}
function onMessage(e){let msg;try{msg=JSON.parse(e.data)}catch{return}if(msg.type==='session'){sessionReady=true;try{localStorage.setItem('ec_session',JSON.stringify({code:msg.roomCode,token:msg.playerToken}))}catch{}refreshResumeButton()}if(msg.type==='state'&&msg.state&&Array.isArray(msg.state.players)){const prev=state;state=msg.state;selectedSeat=null;if(activeDialog===E.confirmOverlay)closeDialog(E.confirmOverlay,false);showGame();render(prev)}if(msg.type==='error'){showToast(msg.message||'通信エラーが発生しました');if(['INVALID_SESSION','SESSION_NOT_FOUND'].includes(msg.code)){try{localStorage.removeItem('ec_session')}catch{}refreshResumeButton();setTimeout(backToLobby,0)}}if(msg.type==='session_required'){sessionReady=false;resumeSession()}if(msg.type==='left')backToLobby()}
...
function resumeSession(){const s=getSession();if(!s||!/^\d{6}$/.test(String(s.code||''))||!/^[a-f0-9]{48}$/.test(String(s.token||''))){try{localStorage.removeItem('ec_session')}catch{}refreshResumeButton();if(state)backToLobby();return}connect(()=>send({type:'resume',code:s.code,token:s.token}))}
...
function finalChoice(){if(!selectedSeat)return;if(!sessionReady){showToast('接続を確認中です…');return}playCue('lock');send({type:state.canSetTrap?'set_trap':'choose_seat',seat:selectedSeat});closeDialog(E.confirmOverlay,false)}
...
function renderGameOver(){
 if(E.gameOver.classList.contains('hidden'))openDialog(E.gameOver,null);const w=state.winnerIndex==null?null:state.players[state.winnerIndex];E.winnerTitle.textContent=w?`${w.name} WIN`:'DRAW';const reasons={target_score:`${state.rules?.targetScore||40}ポイント到達`,forty_points:`${state.rules?.targetScore||40}ポイント到達`,shock_limit:`感電${state.rules?.shockLimit||3}回`,three_shocks:`感電${state.rules?.shockLimit||3}回`,one_seat_left:'イスが残り1脚',opponent_left:'相手が退出',disconnect_timeout:'相手が再接続しませんでした',both_disconnected:'両者が再接続しませんでした'};E.winnerDetail.textContent=`${reasons[state.endReason]||'対戦終了'} / ${state.players[0].score} - ${state.players[1].score} PT`;E.seriesScore.textContent=`SERIES  ${state.players[0].wins} - ${state.players[1].wins}`;
