...
const AI_PROFILES = {
  rei: { id:'rei', name:'レイ', style:'冷徹分析型', avatar:'🧠', colors:['#63d8ff','#6c56ff'], blurb:'履歴を読み、確率の高い手を積み重ねる。' },
  gou: { id:'gou', name:'ゴウ', style:'強気ギャンブラー', avatar:'🔥', colors:['#ff8a45','#ff324f'], blurb:'高得点イスを恐れず狙う攻撃型。' },
  mika:{ id:'mika',name:'ミカ',style:'読心トリックスター',avatar:'🃏',colors:['#e56cff','#55d6ff'],blurb:'裏をかくランダム性と反復読みが得意。' },
  nagi:{ id:'nagi',name:'ナギ',style:'慎重な守備型',avatar:'🧊',colors:['#88eeff','#6f91ff'],blurb:'失点を嫌い、低〜中得点を丁寧に拾う。' }
};
...
function recency(arr,n){let s=0;arr.slice(-5).forEach((x,i)=>{if(x===n)s+=(i+1)});return s}
function aiTrapChoice(room){
  const seats=room.remainingSeats, profile=room.aiProfile, diff=room.aiDifficulty, humanSits=room.history.sits[0];
...
function aiSeatChoice(room){
  ...
  if(diff==='easy') return seats[crypto.randomInt(0,seats.length)];
  return weightedPick(seats,n=>{
    const risk=estimatedHumanTrapProb(room,n); let s=1;
    if(profile==='gou') s=Math.pow(n,1.55)/(1+risk*(diff==='hard'?.35:.18));
    else if(profile==='nagi') s=(n*.9+4)/(1+risk*(diff==='hard'?2.3:1.4)+(ai.score>20?1.2:0));
    else if(profile==='mika') s=(n*1.15+Math.random()*8)/(1+risk*(diff==='hard'?1.25:.7));
    else s=(n*1.35+2)/(1+risk*(diff==='hard'?1.8:.9));
    if(ai.shocks>=2)s/=1+risk*1.8;
    if(ai.score+n>=40)s*=1.75;
    return Math.max(.01,s);
  });
}
function applySeatChoice(room,idx,seat){
...
function scheduleAI(room){
  ...
  if(room.phase==='set_trap'&&room.setterIndex===1){setTimeout(()=>{if(rooms.get(room.code)!==room||room.phase!=='set_trap'||room.setterIndex!==1)return;const seat=aiTrapChoice(room);room.trapSeat=seat;room.history.traps[1].push(seat);room.phase='choose_seat';room.lastResult=null;broadcastState(room)},delay+crypto.randomInt(150,650));}
  if(room.phase==='choose_seat'&&room.sitterIndex===1){setTimeout(()=>{if(rooms.get(room.code)!==room||room.phase!=='choose_seat'||room.sitterIndex!==1)return;applySeatChoice(room,1,aiSeatChoice(room))},delay+crypto.randomInt(250,900));}
}

wss.on('connection',ws=>{
  send(ws,{type:'hello',serverTime:Date.now(),aiProfiles:Object.values(AI_PROFILES)});
  ws.on('message',raw=>{
    let msg;try{msg=JSON.parse(raw.toString())}catch{return fail(ws,'不正なデータです')}
    try{
      ...
      if(msg.type==='create_ai_room'){
        const profile=AI_PROFILES[msg.aiId]||AI_PROFILES.rei,difficulty=DIFFICULTIES.has(msg.difficulty)?msg.difficulty:'normal';
        const{room,player,index}=newRoom(msg.name,'ai');room.aiProfile=profile.id;room.aiDifficulty=difficulty;room.players[1]=newPlayer(profile.name,{isAI:true,connected:true,avatar:profile.avatar});attach(ws,room,player,index);startGame(room);return;
      }
      ...
      if(msg.type==='set_trap'){
        if(room.phase!=='set_trap'||idx!==room.setterIndex)return fail(ws,'今は電気イスを設定できません');const seat=Number(msg.seat);if(!room.remainingSeats.includes(seat))return fail(ws,'そのイスは選べません');room.trapSeat=seat;room.history.traps[idx].push(seat);room.phase='choose_seat';room.lastResult=null;broadcastState(room);scheduleAI(room);return;
      }
      if(msg.type==='choose_seat'){
        if(room.phase!=='choose_seat'||idx!==room.sitterIndex)return fail(ws,'今は着席できません');const seat=Number(msg.seat);if(!room.remainingSeats.includes(seat))return fail(ws,'そのイスは選べません');applySeatChoice(room,idx,seat);return;
      }
      if(msg.type==='rematch_vote'){
        if(room.phase!=='game_over')return fail(ws,'ゲーム終了後に使えます');if(room.mode==='ai'){room.rematchVotes=[true,true];startGame(room);return}room.rematchVotes[idx]=true;if(room.rematchVotes[0]&&room.rematchVotes[1])startGame(room);else broadcastState(room);return;
      }
